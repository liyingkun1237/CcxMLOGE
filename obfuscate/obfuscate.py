...
def obfuscate_global_import_methods(tokens, name_generator, table=None):
    """
...
def obfuscate(module, tokens, options, name_generator=None, table=None):
    ...
    *module* must be the name of the module we're currently obfuscating
    ...
    if options.obfuscate:
        ...
        obfuscate_global_import_methods(tokens, name_generator, table)
        # print("# table: \n%s" % table)
        ...
        if options.obf_builtins:
            obfuscate_builtins(tokens, name_generator, table)


def get_source(filepath, read_encoding='utf-8'):
    '''
    :param filepath:文件地址
    :param read_encoding:读取文件所用编码
    :return:文件具体内容 字符串表示
    '''
    try:
        with open(filepath, encoding=read_encoding) as f:
            source = f.read()
        return source
    except:
        return "fail"
...
def obfuscatefile(projectpath, encoding='utf-8', use_unicode=False, identifier_length=10):
    global name_generator
    try:
        # identifier_length为混淆后替换字符长度,use_unicode表示使用非英文编译
        name_generator = obfuscation_machine(use_unicode=use_unicode, identifier_length=identifier_length)

        # 开始混淆
        for a in os.walk(projectpath):
            file_root = a[0]  # 全路径
            if len(a[-1]) > 0:
                for filename in a[-1]:  # 文件名
                    if str(filename).endswith(".py"):  # 出现py文件 开始混淆
                        pathfile = os.path.join(file_root, filename)
                        source = get_source(pathfile, encoding)
                        if source != 'fail':
                            source = apply_obfuscation(source)  # 混淆代码
                            with open(pathfile, 'w', encoding=encoding) as f:
                                f.write(source)
                        else:
                            print("filepath:" + str(pathfile) + "\t\tcan't read")
...
def copyAndobfuscate(path, copypath, encoding='utf-8', identifier_length=35):
    shutil.copytree(path, copypath)
    obfuscatefile(copypath, encoding=encoding, identifier_length = identifier_length)
    return 'success'


if __name__ == '__main__':
    # print(obfuscatefile("D:/test",encoding='utf-8'))

    # print(obfuscatefile("D:/ccxmodel-0.1.6",encoding='utf-8',identifier_length = 35))
    copyAndobfuscate(r"C:\Users\liyin\Desktop\CcxMLOGE\ccxMLogE", r"C:\Users\liyin\Desktop\CcxMLOGE\TestUnit\ccxMLogE",
                     )
