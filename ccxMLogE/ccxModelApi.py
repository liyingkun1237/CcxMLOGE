...
import time
import json
from ccxMLogE.config import f_mdAllconf, variableurl, modelurl, IP
from ccxMLogE.deployModel import f_regenPort, f_gendeployCode, f_genshellCode, f_runshell, f_varApiDesc, \
    f_updateApiDesc, f_genTestJson, f_jiexiModelre, f_ApiDocWriter
...
def ccxModelApiPredict():
    try:
        ...
        if type == 1:
            ...
            # 4.执行启动脚本
            flag = f_runshell(versionpath)  # True表示服务启动成功，false启动失败
            if not flag:
                return '启动失败的返回'
            # 5.读取原始数据，生成API文档
            df = f_ReadData(base)
            ...
            apiDocPath = os.path.join(versionpath, 'ModelApiDoc_ccx.xlsx')
